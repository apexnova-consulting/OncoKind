...
type Props = {
  ...
  brandFooterDisclaimer?: string | null;
};
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        <p className="text-sm text-slate-600 mt-1">
          Upgrade to the Advocate Plan to download a one-page prep sheet for your oncologist visit.
        </p>
        <Button className="mt-3" asChild>
          <a href="/dashboard/billing">Upgrade to Advocate Plan</a>
        </Button>
      </div>
    );
  }
  ...
  function printablePrepSheetHtml() {
    ...
    const generatorLabel =
      brandDisplayName === 'OncoKind'
        ...
        : `Generated by ${brandDisplayName} · Powered by OncoKind`;

    return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>${brandDisplayName} Doctor Prep Sheet</title>
  <style>
    body { font-family: Arial, sans-serif; color: #0f172a; margin: 32px; line-height: 1.5; }
    .brand { display: flex; align-items: center; gap: 12px; margin-bottom: 8px; }
    .brand img { max-height: 38px; max-width: 120px; object-fit: contain; }
...
<body>
  <div class="brand">
    ${brandLogoUrl ? `<img src="${brandLogoUrl}" alt="${brandDisplayName}" />` : ''}
    <div class="logo">${brandDisplayName}</div>
  </div>
  ...
  async function saveAppointmentForCheckIn() {
    const payload = getCalendarPayload();
    if (!payload) {
      setReminderMsg('Please enter appointment date and time first.');
      return;
    ...
    try {
      const response = await fetch('/api/prep-appointments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          reportId,
          reportTitle,
          appointmentAt: payload.start.toISOString(),
          notes,
        }),
      });
      const data = await response.json();
      if (!response.ok) {
        setReminderMsg(data.error ?? 'Unable to save this appointment right now.');
        return;
      }
      setSavedAppointmentId(data.appointment?.id ?? null);
      setReminderMsg('Appointment saved. OncoKind will show a 24-hour follow-up check-in after your visit.');
    } catch {
      setReminderMsg('Unable to save this appointment right now.');
    } finally {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <Button variant="outline" onClick={saveAppointmentForCheckIn} disabled={savingAppointment}>
      ...
      {savedAppointmentId ? (
        <p className="mt-1 text-xs text-slate-500">
          After your appointment passes, your check-in will appear in Timeline and Reports.
        </p>
      ) : null}
    </div>
  );
